...
namespace OrbisGL.Input
{
    public unsafe class OrbisKeyboard : IKeyboard
    {
        ...
        public bool Initialize(int UserID = Constants.ORBIS_USER_SERVICE_USER_ID_ALL_USERS)
        {
            ...
            if (sceImeKeyboardOpen(UserID, &Param) != Constants.ORBIS_OK)
                return false;

            if (KeyboardLayout == null && sceSystemServiceParamGetInt(Constants.ORBIS_SYSTEM_SERVICE_PARAM_ID_LANG, out int LangID) == Constants.ORBIS_OK)
            {
